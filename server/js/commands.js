module.exports = {
	...
	// GET_MESSAGE: 'SELECT isIn, time FROM userLogs WHERE userId = ? ORDER BY time DESC LIMIT ? OFFSET ?;',
	...
	QUERY_CHECK: `SELECT * FROM userInfo WHERE ((? = '' OR lastName LIKE CONCAT('%', ?, '%')) OR (? = '' OR firstName LIKE CONCAT('%', ?, '%')) OR (? = '' OR middleName LIKE CONCAT('%', ?, '%')) OR (? = '' OR lrn LIKE CONCAT('%', ?, '%')) OR (? = '' OR barangay LIKE CONCAT('%', ?, '%'))) AND ((? IS NULL OR ? = '' OR gradeLevel = ?) AND(? is NULL OR ? = '' OR section = ?));`
